...
// formData (multer.none): multipart/form-data fields → req.body (JSON still works)
// Routes with uploadToS3 already parse multipart (fields + files)
generalRoute.post("/add-suggestion", formData, validateData(addSuggestionSchema), addSuggestion)
...
generalRoute.post("/saveDocument", Authorization, formData, validateData(saveDocumentSchema), saveDocument)
...
generalRoute.post("/multi-remove-follower", Authorization, formData, validateData(multiRemoveFollowerSchema), multiRemoveFollower)
...
generalRoute.post("/follow", Authorization, formData, validateData(followSchema), follow)
